...
from .layers import DGLMessagePassingNetwork
...
class AdaptivePrompter(nn.Module):
    def __init__(self, hidden_dim:int, task_num:int, args_dict:dict):
        ...
        for i in range(task_num):
            prompt = torch.normal(mean=0, std=0.01, size=(task_num,), dtype=torch.float32,
                                  requires_grad=True).to(args_dict['device'])
            prompt.retain_grad()
            self.prompts.append(prompt)
        self.Grad_Matrix = torch.zeros(
            size=(task_num, task_num), device=args_dict['device'])
        self.hidden_dim = hidden_dim
        self.device = args_dict['device']
        self.task_num = task_num
        self.mlp = MLP(input_dim=hidden_dim, output_dim=task_num,dropout_prob=args_dict['drop_rate'],
    ...
    def HierarchicalCluster(self):
        ...
        Z = linkage(self.Grad_Matrix.cpu().numpy(),
                    method='average', metric=cosine_similarity)
...
class Framework(nn.Module):
    ...
    def forward(self, g, add_prompt=True):
        with g.local_scope():
            ...
            # readout
            motif_readout,motif_weights = self.motif_readout(g, ufnf)
            atom_readout,atom_weights = self.atom_readout(g, uaf)
            ##############################
            ...
            if add_prompt == True:
                ...
                atom_pred, motif_pred = self.out_af(
                    atom_representation), self.out_ff(motif_representation)

            return atom_pred, motif_pred, atom_weights, motif_weights
