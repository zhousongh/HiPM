...
def run(args_dict):
    ...
    for seed in range(args_dict['seed'], args_dict['seed'] + args_dict['exp_num']):
        ...
        best_aupr_list.append(best_test_aupr)
    ...
    if args_dict['task_type'] == 'classification':
        print("AUPRC:\n")
        print(np.mean(best_aupr_list), '+-', np.std(best_aupr_list))
...
if __name__ == '__main__':
    ...
    p.add_argument('--dataset', type=str, choices=['Tox21', 'ClinTox', 'ToxCast', 'SIDER', 'QM8', 'QM9'], default='SIDER',
                   help='dataset name')
    p.add_argument('--exp_num', type=int, default=3,
    ...
    p.add_argument('--mode', type=str, default="exp",
                   help='tune or exp')
