...
import numpy as np
from .motif import get_substructures
...
def smiles_to_Molgraph(smiles, 
                         ...
                         use_cycle: bool = True,
                         num_virtual_nodes=0):
    # We follow the dataprocessing method from HimGNN(https://academic.oup.com/bib/article/24/5/bbad305/7245716?login=false)
    mol = Chem.MolFromSmiles(smiles)
    ...
    f_atom_func_group=f_atom_func_group.reshape(-1,1).t()
    for i in range(1,num_func_groups):
        cat_feat=nf[f2a_edges[1][np.argwhere(f2a_edges[0].numpy()==i).ravel()],:].sum(0).reshape(-1,1).t()
    ...
    if len(mol_g.nodes('atom')) != nf.shape[0]:
        return None
    ...
    if len(edge_atom_features_list)==0:
        edge_atom_features=torch.stack([torch.zeros(37, dtype=torch.float32)],dim=0)
    else:
